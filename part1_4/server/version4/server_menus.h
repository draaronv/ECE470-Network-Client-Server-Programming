...
server_message lightList(int option1)
{
...
//Light Menu
int lightMenu(server test,int option)
{
    ...
    while(true)
    {
        lightMenuMess=lightList(option);
        string maLightMenuMess=marshal(lightMenuMess);
        int tempNum=sample.ro.house_room[option].numLights;
...
int roomMenu(server test)
{
    ...
    while(true)
    {
        ...
        {
            int llLightMenu;
            llLightMenu=lightMenu(test,1);
        }
        else if(tempMessage.decision==2)
        {
             int llLightMenu;
            llLightMenu=lightMenu(test,2);

        }
        else if(tempMessage.decision==3)
        {
            int llLightMenu;
            llLightMenu=lightMenu(test,3);
        }
    }
    return -1;
}
